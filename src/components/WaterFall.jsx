...
// 瀑布流单元块组件
import { WFItem_1, WFItem_2 } from './WFItem.jsx';

// 瀑布流状态组件
import { GetData, Loading, ToBottom, NoResult } from './WFStatus.jsx';

// 瀑布流辅助组件
import { ToTop } from './WFUtils.jsx';
...
class WaterFall extends React.Component {
  ...
  _columnNum(w) {
    ...
    _columnNum !== this.state.columnNum &&
      this.setState({
  ...
  _calcShowItem(cb) {
    ...
    // 显示区间不变
    if (this.state.showItemStart === showItemStart && this.state.showItemEnd === showItemEnd) {
      return;
    }
  ...
  toTopShowHide() {
    ...
    document.addEventListener('scroll', function () {
      ...
      ReactDOM.render(
        <ToTop show={self.state.toTop} toTop={self.toTop.bind(self)} />,
  ...
  render() {
    let column_html = this.state.statusFlag === 2 ? '' : this._renderWaterFallItem(),
      ...
      _containerStyle = this.state.wfHeight === 0 ? {} : { height: `${this.state.wfHeight}px` };
